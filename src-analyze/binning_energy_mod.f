        ...
        SUBROUTINE binning_energy(my_m1, my_m2, my_array, &
          my_pot_mon, my_factor, &
          my_file)

        INTEGER :: my_m1, my_m2
        REAL(KIND=dp), DIMENSION(0:f) :: my_array
        REAL(KIND=dp) :: my_pot_mon, my_factor
        ...
        v = f_avg*my_m1 ! length of vector to bin
        ALLOCATE( vector(v) )
        vector = zero
        f0 = f - f_avg
        jv = 0
        DO jf = f0+1, f
          ...
          vector(jv) = &
            my_factor * &
            ( my_array(jf) - REAL(my_m2,dp)*my_pot_mon ) / &
            REAL(my_m2,dp)
        END DO
...
!         CALL FLUSH(6)
        ...
        WRITE(6,120) '# my_m1, my_m2 ', my_m1, my_m2
