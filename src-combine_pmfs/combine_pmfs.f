        PROGRAM combine_pmfs

! Purpose: Line up / combine in overlapping pmfs from n simulations and obtain pmf(z) - pmf(z_0) = -k*T*ln(p(z)/p(z_0))
...
! Program is run as:
!       ../combine_pmfs.exe < combine_pmfs.in >& combine_pmfs.out &
!
! Program reads first the file combine_pmfs.in:
...
! 300.0                       temperature [Kelvin]
        ...
        REAL(KIND=dp), DIMENSION(:), ALLOCATABLE :: pmf ! normalized distribution, pmf

        WRITE(6,'(a)') '# PROGRAM = combine_pmfs'
        CALL FLUSH(6)
...
!         NINT(factor*( REAL(n2,dp)+0.5_dp*(REAL(n2,dp)-REAL(n1,dp)) ))
        z1 = NINT(-10.05_dp*factor)
        z2 = NINT(31.05_dp*factor)
        READ(5,*) width
...
!         jn_char = TRIM(CHAR(jn))
          ...
          DO jj = 1, jmax
            ...
            IF ( ABS( (REAL(jz,dp)/factor) - center_temp ) > eps ) THEN
              ...
              STOP
            ELSE
              IF (ncounts_temp > zero) &
                pmf(jz) = pmf(jz) - temperature*LOG(ncounts_temp)
            END IF
          END DO
...
999       CLOSE(3)
        ...
        WRITE(4,'(a)') '# z  pmf2 [kcal/mol]'
        CALL FLUSH(4)
        DO jz = z1, z2, zstride
          IF ( pmf(jz) > zero ) &
          WRITE(4,400) &
          REAL(jz,dp)/factor, pmf(jz)
          CALL FLUSH(4)
        END DO
        CLOSE(4)
...
400     FORMAT(2(1x,e20.10))

        END PROGRAM combine_pmfs
