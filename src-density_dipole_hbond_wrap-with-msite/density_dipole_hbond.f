...
! 4             a (number of atoms per molecule) (water with one M site, like in DC potential)
! 15.0 15.0 71.44  cell ABC values [Angstrom]
! 0.10 OR ... OR 71.44    dz [Angstrom] (cell(d)=71.44 means sum everything in one z bin)
! 15.99491463 1.0078250321 1.0078250321 0.0  mass(:)
...
! 999.0 OR 1.007         delta (width of the interfacial region, Ang) (from tanh fit in gnuplot)
        ...
        my_d_chrg = 1
        my_d = my_d_chrg 
        ALLOCATE( chrg(0:f,m,a,my_d) )
        CALL reading(my_d, chrg, my_file)

        my_file = '../run-01_dipl.xyz'
        my_file = TRIM(my_file)
        my_d = d
        ALLOCATE( dipl(0:f,m,a,my_d) )
        CALL reading(my_d, dipl, my_file)

        name3='dipole'
        my_file='dipl_mol_with_z.dat'
        CALL get_dipole(name3,my_d_chrg, my_file)
