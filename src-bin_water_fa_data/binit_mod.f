        ...
        SUBROUTINE binit(my_d, my_m, my_n, my_array, my_file)

        INTEGER :: my_d, my_m, my_n
        REAL(KIND=dp), DIMENSION(my_m, my_n) :: my_array
        ...
        WRITE(6,'(2a)') '# SUBROUTINE = binit; filename = ', &
          TRIM(my_file)
        CALL FLUSH(6)

        v = my_m ! length of vector to bin
        ALLOCATE( vector(v) )
        vector = zero
        vector(:) = my_array(:, my_d)
