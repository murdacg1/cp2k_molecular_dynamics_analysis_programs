...
! Purpose: Analyze a CP2K simulation of [Hydroxylated surface + (H20)n)] + [single adsorbate]
! Program is run as:
...
! 0.4           dt      time step [fs]
! 24            n_si    number of Si atoms
! 1             m_surf  number of surface molecules
! 96            a_surf  number of atoms in the hydroxylated surface (increase if have WCs)
! 8             m_wat   number of water molecules
! 3             a_wat   number of atoms per water molecule (increase if have WCs)
! 1             m_sol   number of solute molecules
! 2             a_sol   number of atoms per solute molecule (increase if have WCs)
! 9.82 8.504 32.165                 cell(d)   ABC values [Angstrom]
! 250.0                             temperature [Kelvin]
! -1139.3762346133 -15.5178224408   pot_a pot_b   pot_hydro_surface_and_water pot_adsorbate energies of the monomers [Hartree]
...
! 120 102 Cl1_O10.dat
        ...
        a = m_surf*a_surf + m_wat*a_wat + m_sol*a_sol ! total number atoms
        ALLOCATE( cell(d) )
        cell = zero
        READ(5,*) cell(:)
        READ(5,*) temperature
        ...
        WRITE(6,100) '# f = ', f
        WRITE(6,200) '# dt = ', dt
        WRITE(6,100) '# n_si = ', n_si
        WRITE(6,100) '# m = ', m
        WRITE(6,100) '# a = ', a
        WRITE(6,100) '# m_surf = ', m_surf
        WRITE(6,100) '# a_surf = ', a_surf
        ...
        WRITE(6,100) '# iflag_shift = ', iflag_shift
        ...
        ALLOCATE( symbol(a), symbol_surf(a), symbol_wat(a), symbol_sol(a) )
        ALLOCATE( mass(a), mass_surf(a), mass_wat(a), mass_sol(a) )

!   Trajectory: read in and write out shifted position in each frame

        my_file='../quartz-pos-1.xyz'
        my_d=d
        ALLOCATE( coor(0:f,m,a,my_d) )
        ALLOCATE( coor_surf(0:f,m_surf,a_surf,my_d) )
        ALLOCATE( coor_wat(0:f,m_wat,a_wat,my_d) )
        ALLOCATE( coor_sol(0:f,m_sol,a_sol,my_d) )
        CALL reading(iflag_shift, my_d, coor, coor_surf, coor_wat, coor_sol, &
          my_file)
        ...
        my_a=a
        my_d=d
        CALL writing_all(my_m, my_a, my_d, coor, symbol, &
     &    my_file)
        ...
        my_a=a_wat
        my_d=d
        CALL writing_all(my_m, my_a, my_d, coor_wat, symbol_wat, &
     &    my_file)
...
! selected interatomic distances
        ...
        DEALLOCATE( cell )
        DEALLOCATE( tim, pot )
        DEALLOCATE( symbol, symbol_surf, symbol_wat, symbol_sol)
        DEALLOCATE( mass, mass_surf, mass_wat, mass_sol )
        DEALLOCATE( coor, coor_surf, coor_wat, coor_sol )
